...
public enum EmissionGoalCRUDQueries {
  CREATE("INSERT INTO emission_goals (goal_id, user_id, target_emission, start_date, end_date, status) VALUES (?, ?, ?, ?, ?, ?::status)"),
  READ_ALL("SELECT * FROM emission_goals"),
  READ_BY_ID("SELECT * FROM emission_goals WHERE goal_id = ?"),
  UPDATE("UPDATE emission_goals SET target_emission = ?, start_date = ?, end_date = ?, status = ?::status WHERE goal_id = ?"),
